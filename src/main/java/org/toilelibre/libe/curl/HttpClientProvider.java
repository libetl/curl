...
final class HttpClientProvider {
    ...
    private static void addClientCredentials (final SSLContextBuilder builder, final CertFormat certFormat,
            final String caCertFilePath, final String certFilePath, final CertFormat keyFormat,
            final String keyFilePath, final String password) throws CurlException {
        try {
            ...
            final KeyStore keyStore = HttpClientProvider.generateKeyStore (certFormat, caCertFileObject, certFileObject,
                    keyFormat, keyFileObject, password == null ? null : password.toCharArray ());
    ...
    private static KeyStore generateKeyStore (final CertFormat certFormat, final File caCertFileObject,
            final File certFileObject, final CertFormat keyFormat, final File keyFileObject,
        ...
        final List<Certificate> caCertificatesNotFiltered = caCertFileObject == null ? Collections.emptyList ()
                : certFormat.generateCredentialsFromFileAndPassword (Kind.CERTIFICATE,
                        IOUtils.toByteArray (caCertFileObject), passwordAsCharArray);
        final List<Certificate> caCertificatesFiltered = caCertificatesNotFiltered.stream ()
                .filter ( (certificate) -> (certificate instanceof X509Certificate)
                        && (((X509Certificate) certificate).getBasicConstraints () != -1))
                .collect (Collectors.toList ());
        final List<Certificate> certificates = certFormat.generateCredentialsFromFileAndPassword (Kind.CERTIFICATE,
                IOUtils.toByteArray (certFileObject), passwordAsCharArray);
        final List<PrivateKey> privateKeys = keyFormat.generateCredentialsFromFileAndPassword (Kind.PRIVATE_KEY,
                IOUtils.toByteArray (keyFileObject), passwordAsCharArray);
        ...
        IntStream.range (0, certificates.size ())
                .forEach (i -> HttpClientProvider.setCertificateEntry (keyStore, certificates, i));
        IntStream.range (0, caCertificatesFiltered.size ())
                .forEach (i -> HttpClientProvider.setCaCertificateEntry (keyStore, certificates, i));
        IntStream.range (0, privateKeys.size ()).forEach (i -> HttpClientProvider.setPrivateKeyEntry (keyStore,
                privateKeys, passwordAsCharArray, certificatesAsArray, i));
        return keyStore;
    ...
    private static void handleSSLParams (final CommandLine commandLine, final HttpClientBuilder executor)
        ...
        if (commandLine.hasOption (Arguments.CERT.getOpt ())) {
            ...
            final String cacert = commandLine.getOptionValue (Arguments.CA_CERT.getOpt ()) == null ? null
                    : commandLine.getOptionValue (Arguments.CA_CERT.getOpt ());
            ...
            HttpClientProvider.addClientCredentials (builder, certFormat, cacert, credentials [0], keyFormat, key,
    ...
    private static void setCaCertificateEntry (final KeyStore keyStore, final List<Certificate> certificates,
            final int i) {
        try {
    ...
    private static void setCertificateEntry (final KeyStore keyStore, final List<Certificate> certificates,
            final int i) {
        try {
