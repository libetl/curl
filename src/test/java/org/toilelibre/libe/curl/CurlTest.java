...
public class CurlTest {
    ...
    private HttpResponse curl (String requestCommand) {
        return Curl.curl (String.format(requestCommand, RequestMonitor.port ()));
    }
    
    private String $ (String requestCommand) {
    ...
    public void curlRoot () {
        assertOk(curl("-k --cert src/main/resources/client.p12:password https://localhost:%d/public/"));
    }
    
    @Test
    public void curlToRedirectionWithoutFollowRedirectParam () {
        assertFound(curl("-k https://localhost:%d/public/redirection"));
    }
    
    @Test
    public void curlToUnauthorized () {
        assertUnauthorized(curl("-k https://localhost:%d/public/unauthorized"));
    }

    @Test
    public void curlToRedirectionWithFollowRedirectParam () {
        assertOk(curl("-k -L https://localhost:%d/public/redirection"));
    }
    
    @Test
    public void curlWithHeaders () {
        assertOk(curl("-k -H'Host: localhost' -H'Authorization: 45e03eb2-8954-40a3-8068-c926f0461182' https://localhost:%d/public/v1/coverage/sncf/journeys?from=admin:7444extern"));
    }
    
    @Test
    public void readCurlPublicRoot () {
        assertOk(curl($ ("-k https://localhost:%d/public/")));
    }
    ...
    public void readCurlCommand () {
        assertOk(curl("-k -X GET -H 'User-Agent: curl/7.49.1' -H 'Accept: */*' -H 'Host: localhost'  'https://localhost:%d/public/curlCommand1?param1=value1&param2=value2'"));
    }
    
    @Test
    public void readCurlOfCurlCommand () {
        assertOk(curl($ ("-k -X GET -H 'User-Agent: curl/7.49.1' -H 'Accept: */*' -H 'Host: localhost'  'https://localhost:%d/public/curlCommand2?param1=value1&param2=value2'")));
    }
    
    @Test
    public void tryToLoginAnonymouslyWithCurlCommand () {
        assertUnauthorized(curl("-k -X GET -H 'User-Agent: curl/7.49.1' -H 'Accept: */*' -H 'Host: localhost'  'https://localhost:%d/private/login'"));
    }
    
    @Test
    public void loginWithIncorrectLoginCurlCommand () {
        assertUnauthorized(curl("-k -X GET -H 'User-Agent: curl/7.49.1' -H 'Accept: */*' -H 'Host: localhost' -u foo:bar 'https://localhost:%d/private/login'"));
    }
    
    @Test
    public void loginCorrectLoginCurlCommand () {
        assertOk(curl("-L -k -X GET -H 'User-Agent: curl/7.49.1' -H 'Accept: */*' -H 'Host: localhost' -u user:password 'https://localhost:%d/private/login'"));
    }
    
    @Test
    public void withJsonBody () {
        assertOk(curl("-k -X POST 'https://localhost:%d/public/json' -d '{\"var1\":\"val1\",\"var2\":\"val2\"}'"));
    }
